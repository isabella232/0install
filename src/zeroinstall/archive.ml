...
let check_type_ok system =
  ...
  function
    ...
    | "application/x-zstd-compressed-tar" -> if missing "unzstd" then
        Safe_exn.failf "This package looks like a zstd-compressed package, but you don't have the \"unzstd\" command \
                    I need to extract it. Install the package containing it (it's probably called \"zstd\") first."
